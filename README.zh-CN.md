...
**1. 投递原稿，流水线自动完成**——上传小说 `.txt` 或漫画 `.zip/.cbz`，五步流程自动执行：识别类型 → OCR / 分段 → 本地翻译 → 修复回嵌 → 打包产物。运行状态卡实时展示模型就绪情况与局域网访问地址。

![首页：投递原稿与运行状态](docs/assets/screenshot-home.png)

**2. 实时处理队列**——每个任务都有进度卡片，产物保留在 `data/output/`，并附完整 `translation-report.json`。

![处理队列：任务完成](docs/assets/screenshot-jobs.png)

**3. ⭐ 本地 AI 项目客服「青芽」——装在你电脑里的技术支持。** 点击右下角的青芽，它开口第一句就报出你*真实的*运行环境：系统、CPU/GPU、模型状态、最近任务情况——因为回答前它会先读取本地诊断信息。把 PowerShell 输出或报错日志直接粘给它，它会解释问题并给出修复建议。所有对话都走本机 Ollama 模型：**你的提问和日志一个字都不会上传**，就像你的翻译内容一样。

![青芽助手：本地 AI 诊断对话](docs/assets/screenshot-assistant.png)
...
原来的 Marian/OPUS 日中模型只适合短句直译，缺少小说上下文和口语风格控制。当前默认改为 Ollama `qwen3.5:2b`，并启用 ACGN 口语化提示与前文上下文；内存充足时建议改用 `qwen3.5:9b`，译文质量明显更好：
...
- 漫画识别基于 EasyOCR CRAFT 检测 + Ollama 视觉模型（glm-ocr）识别；极小假名和拟声词仍可能漏检，CPU 推理速度较慢。
